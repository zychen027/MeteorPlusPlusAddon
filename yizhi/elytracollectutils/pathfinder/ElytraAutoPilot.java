package com.zychen027.meteorplusplus.modules.elytracollectutils.pathfinder;
        ...
        if (!onTakeoff) {
            if (player != null) {
                ...
                if (itemChest != Items.ELYTRA) {
                    player.sendMessage(Text.translatable("text.pathfinder.takeoffFail.noElytraEquipped").formatted(Formatting.RED), true);
                    return;
                }
                if (elytraDamage == 1) {
                    player.sendMessage(Text.translatable("text.pathfinder.takeoffFail.elytraBroken").formatted(Formatting.RED), true);
                    return;
                }
                if (itemMain != Items.FIREWORK_ROCKET && itemOff != Items.FIREWORK_ROCKET ) {
                    player.sendMessage(Text.translatable("text.pathfinder.takeoffFail.fireworkRequired").formatted(Formatting.RED), true);
                    return;
                }
                ...
                for (double i = c; i < l; i++) {
                    ...
                    if (!world.getBlockState(blockPos).isAir()) {
                        player.sendMessage(Text.translatable("text.pathfinder.takeoffFail.clearSkyNeeded").formatted(Formatting.RED), true);
                        return;
                    }
                    n++;
                }
        ...
        if (player != null) {
            if (groundheight > ModConfig.INSTANCE.minHeight) {
                ...
                if (isChained) {
                    isflytoActive = true;
                    isChained = false;
                    minecraftClient.inGameHud.getChatHud().addMessage(Text.translatable("text.pathfinder.flyto", argXpos, argZpos).formatted(Formatting.GREEN));
                }
                return;
            }
            ...
            if (!hasFirework) {
                if (!tryRestockFirework(player)) {
                    ...
                    minecraftClient.options.jumpKey.setPressed(false);
                    onTakeoff = false;
                    player.sendMessage(Text.translatable("text.pathfinder.takeoffAbort.noFirework").formatted(Formatting.RED), true);
                    doGlide = true;
                }
            }
...
//        }
        ...
        if (autoFlight) {
            ...
            if (isflytoActive || forceLand) {
                if (isLanding || forceLand) {
                    ...
                    if (distance < 20) {
                        minecraftClient.player.sendMessage(Text.translatable("text.pathfinder.landing").formatted(Formatting.BLUE), true);
    ...
    private void onClientTick() //20 times a second, before first screen tick
    {
        ...
        if (!landPressed && KeyBindings.landBinding.isPressed() && autoFlight) {
            player.sendMessage(Text.translatable("text.pathfinder.landing").formatted(Formatting.BLUE), true);
            SoundEvent soundEvent = SoundEvent.of(Identifier.of(ModConfig.INSTANCE.playSoundOnLanding));
            player.playSound(soundEvent, 1.3f, 1f);
            minecraftClient.options.useKey.setPressed(false);
            forceLand = true;
        ...
        if(!configPressed && KeyBindings.configBinding.isPressed()) {
            if (player.isGliding()) {
                if (!autoFlight && groundheight < ModConfig.INSTANCE.minHeight){
                    player.sendMessage(Text.translatable("text.pathfinder.autoFlightFail.tooLow").formatted(Formatting.RED), true);
                    doGlide = true;
                }
                else {
