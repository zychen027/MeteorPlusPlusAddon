...
class KillAura : Module(
        ...
        if (reset.get()) {
            ...
            if (packet is PlayerInteractEntityC2SPacket) {
                ...
                try {
                    ...
                    val typeHandler = modeField[packet]
                    val typeField = typeHandler.javaClass.getDeclaredField("type")
                    typeField.isAccessible = true
                    val type = typeField[typeHandler]
                    if (type.toString() == "ATTACK") {
                        tick.reset()
                    }
                } catch (e: Exception) {
                    // 忽略反射错误
    ...
    private fun doAura() {
        ...
        if (autoSwitch.get() != SwitchMode.None && !itemInHand()) {
            val predicate: (ItemStack) -> Boolean = when (weapon.get()) {
                Weapon.Axe -> { stack -> stack.item is AxeItem }
        ...
        if (!itemInHand() && autoSwitch.get() != SwitchMode.Silent) return
